...
impl OrphanBlockPool {
    ...
    /// 고아 풀에 블록을 추가합니다.
    /// `requested_missing_chunks`: 고아에 대해 누락된 청크가 요청되었는지 여부입니다.
    ...
    /// 블록 `prev_hash`가 "채택"할 수 있는 풀의 모든 고아, 즉 `prev_hash`의 자식들을 제거하고 리스트를 반환합니다.
    /// 이 함수는 `prev_hash`가 수락될 때 호출되며, 따라서 그 자식들은 고아 풀에서 제거되어 처리될 수 있습니다.
    ...
    /// 블록 `parent_hash`의 `target_depth` 직계 자손에 속하는 고아 목록을 반환합니다.
...
/// 고아에 대한 청크를 요청하는 데 필요한 정보를 포함합니다. 필드는 `request_chunks_for_orphan`의 인수로 사용됩니다.
...
fn check_known_store(
    ...
    //log("check_known_store ()"); // 블록 생성 마다 계속 찍힘.
...
impl Chain {
    pub fn make_genesis_block(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("make_genesis_block() in impl Chain");
    ...
    pub fn new_for_view_client(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("new_for_view_client() in impl Chain");
        ...
        println!("let genesis = Self::make_genesis_block(&*runtime_adapter, chain_genesis)?;");
        let (sc, rc) = unbounded();
        Ok(Chain {
            store,
            runtime_adapter,
    ...
    /// requesting it.
    ///
    /// "헤더 우선" 블록 전파의 일부로 블록 헤더를 처리합니다.
    /// 헤더의 유효성을 검사하지만 헤더를 저장하거나 헤더 헤드를 업데이트하지 않습니다.
    /// 저장하거나 이를 기반으로 업데이트하지 않습니다. 블록을 요청한 후 블록을 다시 받으면 이를 업데이트할 것입니다.
    /// 블록을 반환받으면 업데이트합니다.
