...
impl BlocksInProcessing {
    ...
    pub(crate) fn contains(&self, block_hash: &CryptoHash) -> bool {
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("contains() in BlocksInProcessing");
    ...
    pub(crate) fn remove(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("remove() in BlocksInProcessing");
    ...
    pub(crate) fn add_dry_run(&self, block_hash: &CryptoHash) -> Result<(), AddError> {
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("add_dry_run() in BlocksInProcessing:블록을 추가하는 것이 아니라 단순히 블록을 추가할 수 있는지 확인합니다.");
    ...
    pub(crate) fn wait_for_all_blocks(&self) -> bool {
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("wait_for_all_blocks() in BlocksInProcessing:모든 블록에 대해 apply_chunks_done이 true로 표시될 때까지 기다립니다. 새 블록에 청크 적용이 완료되면 true를 반환합니다.");
    ...
    pub(crate) fn wait_for_block(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("wait_for_block() in BlocksInProcessing:`block_hash` 블록에 대해 apply_chunks_done이 true로 표시될 때까지 기다립니다.");
        let _ = self
            .preprocessed_blocks
            .get(block_hash)
