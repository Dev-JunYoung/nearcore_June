...
impl BlockSync {
    pub fn new(
        ...
        file_write("new() impl BlockSync").unwrap();
        BlockSync {
            network_adapter,
            last_request: None,
            block_fetch_horizon,
    ...
    pub fn run(
        ...
        file_write("run() impl BlockSync").unwrap();
    ...
    fn check_state_needed(&self, chain: &Chain) -> Result<bool, near_chain::Error> {
        file_write("check_state_neededO() impl BlockSync").unwrap();
        let head = chain.head()?;
    ...
    fn block_sync(
        ...
        file_write("block_sync() impl BlockSync").unwrap();
    ...
    fn block_sync_due(&mut self, chain: &Chain) -> Result<bool, near_chain::Error> {
        file_write("block_sync_due() impl BlockSync").unwrap();
