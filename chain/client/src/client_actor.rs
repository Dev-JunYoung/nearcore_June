...
impl Handler<WithSpanContext<Status>> for ClientActor {
    ...
    fn handle(&mut self, msg: WithSpanContext<Status>, ctx: &mut Context<Self>) -> Self::Result {
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("handle() impl Handler<WithSpanContext<Status>> for ClientActor");
        let (_span, msg) = handler_debug_span!(target: "client", msg);
...
impl Handler<WithSpanContext<GetNetworkInfo>> for ClientActor {
    ...
    fn handle(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("handle() impl Handler<WithSpanContext<GetNetworkInfo>> for ClientActor");
        self.check_triggers(ctx);
...
impl ClientActor {
    ...
    /// 최신 높이를 가져와서 다음 블록을 생성해야 하는지 확인합니다.
    /// 그렇지 않으면 블록이 도착할 때까지 기다리거나 타임아웃 후 건너뛰도록 제안합니다.
    fn handle_block_production(&mut self) -> Result<(), Error> {
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("handle_block_production()");
    ...
    /// "트리거"의 예약된 시간이 지났는지 확인하고, 지났다면 트리거를 호출합니다.
    /// 트리거는 한 단계의 상태 동기화를 실행하거나 블록을 생성할 수 있는지 확인하는 등 클라이언트의 중요한 기능입니다.
    /// Actix 메시지를 처리하기 전과 schedule_triggers에서도 호출됩니다.
    /// 이는 모든 트리거가 액틱스 메시지보다 높은 우선순위를 갖도록 하기 위함입니다.
    /// 그렇지 않으면 Actix 라이브러리의 버그로 인해 메일함에 메시지가 있는 동안 Actix가 메시지 처리의 우선순위를 정합니다. 그래서 Actix에서는 `run_later` 대신 커스텀 `run_timer` 함수를 사용하여 스케줄링 트리거를 처리합니다.
    /// 다음 `check_triggers`가 호출될 때까지의 지연 시간(최소(가장 가까운 트리거까지의 시간, 1초))을 반환합니다.
    ...
    fn check_triggers(&mut self, ctx: &mut Context<ClientActor>) -> Duration {
        ...
        if let Some(config_updater) = &mut self.config_updater {
            config_updater.try_update(&|updateable_client_config| {
                print!("line: {},{:?} ",std::line!(), std::thread::current().id());
                log("self.client.update_client_config(updateable_client_config)");
        ...
        if self.sync_started {
            /*print!("line: {},{:?} ",std::line!(), std::thread::current().id());
            print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("if self.sync_started == true // in check_triggers()");*/
    ...
    fn try_handle_block_production(&mut self) {
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("try_handle_block_production()");
    ...
    /// 주어진 `next_height` 높이에 대한 블록 생성자라면 블록을 생성합니다.
    /// 에러를 반환할 수 있으며, 에러를 처리하고 스케줄을 재조정하려면 `produce_block`과 함께 호출해야 합니다.
    ...
    fn get_apply_chunks_done_callback(&self) -> DoneApplyChunkCallback {
        //log("get_apply_chunks_done_callback()");
        ...
        match sync {
            ...
            | SyncRequirement::AdvHeaderSyncDisabled => {
                if currently_syncing {
                    print!("line: {},{:?} ",std::line!(), std::thread::current().id());
                    log("match sync in run_sync_step()");
