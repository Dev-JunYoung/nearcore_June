...
impl Client {
    pub(crate) fn update_client_config(&self, update_client_config: UpdateableClientConfig) {
        ...
        log("update_client_config() in impl Client");
...
impl Client {
    pub fn new(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("new() in impl Client:클라이언트 생성");
        file_write("new()").unwrap();
    ...
    // 헤드가 마지막으로 업데이트된 이후 최소 `stall_timeout`이 지났는지 확인합니다.
    // 이 메서드가 호출되었는지 확인합니다. 그렇다면 현재 헤드를 다시 브로드캐스트합니다.
    pub fn check_head_progress_stalled(&mut self, stall_timeout: Duration) -> Result<(), Error> {
        file_write("check_head_progress_stalled():헤드가 업데이트 된 후, 최소 stall_timeout 지났어?").unwrap();
    ...
    pub fn reintroduce_transactions_for_block(&mut self, me: AccountId, block: &Block) {
        file_write("reintroduce_transactions_for_block()").unwrap();
        for (shard_id, chunk_header) in block.chunks().iter().enumerate() {
            let shard_id = shard_id as ShardId;
            if block.header().height() == chunk_header.height_included() {
                if cares_about_shard_this_or_next_epoch(
    ...
    fn known_block_height(&self, next_height: BlockHeight, known_height: BlockHeight) -> bool {
        file_write("known_block_height()").unwrap();
        #[cfg(feature = "test_features")]
        {
            file_write("known_block_height():이 블록 높이가 아직 알려지지 않았는지 확인합니다.").unwrap();
    ...
    fn is_me_block_producer(
        ...
        file_write("is_me_block_producer():우리가 다음 블록 생성자인지 확인합니다.").unwrap();
        #[cfg(feature = "test_features")]
        {
    ...
    fn should_reschedule_block(
        ...
        file_write("should_reschedule_block()").unwrap();
    ...
    pub fn get_chunk_headers_ready_for_inclusion(
        ...
        file_write("get_chunk_headers_ready_for_inclusion()").unwrap();
    ...
    pub fn num_chunk_headers_ready_for_inclusion(
        ...
        file_write("num_chunk_headers_ready_for_inclusion():포함할 준비가 된 청크 헤더 수").unwrap();
        let entries =
    ...
    /// 주어진 `next_height` 블록 높이에 대한 블록 생성자인 경우 블록을 생성합니다.
    /// 생성된 블록(적용되지 않음)을 반환하거나 에러를 반환합니다.
    pub fn produce_block(&mut self, next_height: BlockHeight) -> Result<Option<Block>, Error> {
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("produce_block()");
        file_write("produce_block():주어진 `next_height` 블록 높이에 대한 블록 생성자인 경우 블록을 생성합니다.").unwrap();
    ...
    pub fn produce_chunk(
        ...
        file_write("produce_chunk() :청크 생성").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("produce_chunk()");
    ...
    fn maybe_insert_invalid_transaction(
        ...
        file_write("maybe_insert_invalid_transaction()").unwrap();
        if insert {
    ...
    /// 트랜잭션 풀에서 limit 까지 유효한 트랜잭션의 정렬된 목록을 준비합니다.
    fn prepare_transactions(
        ...
        file_write("prepare_transactions():트랜잭션 풀에서 limit 까지 유효한 트랜잭션의 정렬된 목록을 준비합니다.").unwrap();
    ...
    pub fn send_challenges(&mut self, challenges: Vec<ChallengeBody>) {
        file_write("send_challenges()").unwrap();
    ...
    /// 수신된 블록을 처리합니다. 블록 헤더가 유효하지 않거나 블록의 형태가 잘못된 경우 피어를 금지합니다.
    ...
    pub fn receive_block(
        ...
        file_write("receive_block():수신된 블록을 처리합니다. 블록 헤더가 유효하지 않거나 블록의 형태가 잘못된 경우 피어를 금지합니다.").unwrap();
        let hash = *block.hash();
    ...
    /// 수신된 블록을 처리합니다.
    /// 이 함수는 먼저 블록을 여러 번 처리하지 않도록 블록 높이에 따라 사전 검사를 수행합니다.
    /// 그런 다음 블록 헤더를 처리합니다. 헤더가 유효하면 해당 블록을 피어에게 브로드캐스트합니다. 그런 다음 블록 처리 프로세스를 시작하여 전체 블록을 처리합니다.
    pub(crate) fn receive_block_impl(
        ...
        file_write("receive_block_impl():수신된 블록 처리").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("receive_block_impl()");
    ...
    fn check_block_height(
        ...
        file_write("check_block_height():스팸 방지를 위한 블록 높이 사전 검사 수행.(높이 유효?->true)").unwrap();
        let head = self.chain.head()?;
    ...
    fn verify_and_rebroadcast_block(
        ...
        file_write("verify_and_rebroadcast_block()").unwrap();
    ...
    ///
    /// 블록 처리를 시작합니다. 청크 적용은 레이온 스레드 풀에서 비동기적으로 이루어지기 때문에 이 함수는 전체 처리가 완료되기 전에 반환된다는 점에 유의하세요.
    /// 'apply_chunks_done_callback': 청크 적용이 완료될 때 호출되는 콜백입니다.
    pub fn start_process_block(
        ...
        file_write("start_process_block()").unwrap();
    ...
    /// blocks.
    /// 청크 적용이 완료된 블록이 있는지 확인하고, 이 블록에 대해 포스트 프로세싱을 실행합니다.
    /// 블록에 대한 포스트 프로세싱을 실행합니다.
    pub fn postprocess_ready_blocks(
        &mut self,
        apply_chunks_done_callback: DoneApplyChunkCallback,
        should_produce_chunk: bool,
    ) -> (Vec<CryptoHash>, HashMap<CryptoHash, near_chain::Error>) {
        file_write("postprocess_ready_blocks():청크 적용이 완료된 블록 확인, 블록에 대한 포스트 프로세싱 실행.").unwrap();
        let me = self
            .validator_signer
            .as_ref()
            .map(|validator_signer| validator_signer.validator_id().clone());
    ...
    /// 체인에서 블록 처리 결과를 처리하고 다음을 포함하여 체인에서 수행 할 수없는 단계를 완료합니다.
    /// - 챌린지 전송
    /// - 누락된 청크 요청
    pub(crate) fn process_block_processing_artifact(
        ...
        file_write("process_block_processing_artifact():").unwrap();
    ...
    fn ban_chunk_producer_for_producing_invalid_chunk(
        ...
        file_write("ban_chunk_producer_for_producing_invalid_chunk()").unwrap();
        let epoch_id =
    ...
    fn rebroadcast_block(&mut self, block: &Block) {
        file_write("rebroadcast_block()").unwrap();
    ...
    pub fn on_chunk_completed(
        ...
        file_write("on_chunk_completed() : ShardsManager가 청크 처리를 완료하면 비동기적으로 호출됩니다.").unwrap();
    ...
    pub fn on_invalid_chunk(&mut self, encoded_chunk: EncodedShardChunk) {
        file_write("on_invalid_chunk()").unwrap();
    ...
    pub fn on_chunk_header_ready_for_inclusion(
        ...
        file_write("on_chunk_header_ready_for_inclusion()").unwrap();
    ...
    pub fn sync_block_headers(
        ...
        file_write("sync_block_headers()").unwrap();
    ...
    /// 둠슬러그에 알려진 최신 해시가 현재 헤드와 일치하는지 확인하고, 일치하지 않으면 업데이트합니다.
    pub fn check_and_update_doomslug_tip(&mut self) -> Result<(), Error> {
        file_write("check_and_update_doomslug_tip():둠슬러그에서 알려진 최신 해시가 현재 헤드와 일치확인. if no->업데이트").unwrap();
    ...
    pub fn sandbox_update_tip(&mut self, height: BlockHeight) -> Result<(), Error> {
        file_write("sandbox_update_tip()").unwrap();
        let tip = self.chain.head()?;
    ...
    pub fn sandbox_delta_time(&self) -> chrono::Duration {
        file_write("sandbox_delta_time()").unwrap();
    ...
    pub fn send_approval( //#Consensus#messages? //메시지
        &mut self,
        parent_hash: &CryptoHash,
        approval: Approval,
    ) -> Result<(), Error> {
        file_write("send_approval()").unwrap();
        let next_epoch_id = self.runtime_adapter.get_epoch_id_from_prev_block(parent_hash)?; //이전 블록의 해시로부터 에포크 ID를 가져옵니다.
        let next_block_producer =
            //get_block_producer:메인 블록의 주어진 높이에 대한 블록 생성자
    ...
    /// 블록이 수락될 때 호출됩니다.
    /// 스킵_프로듀스_청크`가 거짓일 때만 청크를 생성합니다.
    /// 블록에 누락된 청크가 있을 때를 시뮬레이션하기 위해 `skip_produce_chunk`를 true로 설정합니다.
    pub fn on_block_accepted_with_optional_chunk_produce(
        ...
        file_write("on_block_accepted_with_optional_chunk_produce() : 블록이 수락될 때 호출됩니다.").unwrap();
    ...
    pub fn persist_and_distribute_encoded_chunk(
        ...
        file_write("persist_and_distribute_encoded_chunk()").unwrap();
    ...
    pub fn request_missing_chunks(
        ...
        file_write("request_missing_chunks()").unwrap();
    ...
    pub fn process_blocks_with_missing_chunks(
        ...
        file_write("process_blocks_with_missing_chunks() : 누락된 청크가 있는 블록이 처리할 준비가 되었는지 확인합니다.").unwrap();
        let me =
    ...
    pub fn is_validator(&self, epoch_id: &EpochId, block_hash: &CryptoHash) -> bool {
        file_write("is_validator(): validator?").unwrap();
    ...
    fn handle_process_approval_error(
        ...
        file_write("handle_process_approval_error()").unwrap();
        let is_validator =
            |epoch_id,
             block_hash,
             account_id,
    ...
    /// 블록 승인을 수집합니다.
    ///
    /// 현재 팁의 에포크가 주어지면 승인을 doomslug로 보냅니다:
    /// 1. 우리는 해당 팁의 에포크에서 목표 높이에 대한 블록 생성자입니다;
    /// 2. 서명이 계정의 서명과 일치합니다;
    /// 만약 우리가 블록 생성자가 아니지만 이전 블록을 모른다면, 우리는
    /// 승인은 다음 에포크의 승인일 수 있으므로 `pending_approvals`에 추가합니다.
    ///
    /// # 인수
    /// * `approval` - 수집할 승인
    /// 승인 유형 - 방금 생성된 승인인지 여부 (이 경우 유효성 검사를 건너뜁니다,
    /// 다음 블록 생성자인지 여부만 확인하여 둠슬러그에 저장합니다.)
    pub fn collect_block_approval(&mut self, approval: &Approval, approval_type: ApprovalType) { //#Consensus#messages 메시지
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("collect_block_approval()");
        file_write("collect_block_approval() :블록 승인을 수집합니다.").unwrap();
        ...
        let parent_hash = match inner {
            ...
            ApprovalInner::Skip(parent_height) => {
                // 에포크 ID의 해시맵 반환 current(height, epoch_id)
        ...
        let next_block_epoch_id =
            //get_epoch_id_from_prev_block:이전 블록의 해시로부터 에포크 ID를 가져옵니다.
    ...
    fn possibly_forward_tx_to_next_epoch(&mut self, tx: &SignedTransaction) -> Result<(), Error> {
        file_write("possibly_forward_tx_to_next_epoch() :다음 에포크의 검증자에게 전달합니다.").unwrap();
        let head = self.chain.head()?;
    ...
    fn process_tx_internal(
        ...
        file_write("process_tx_internal() :트랜잭션을 처리하고 멤풀에 추가하거나 다른 유효성 검사기로 리디렉션하기 위해 반환합니다.").unwrap();
        let head = self.chain.head()?;
        let me = self.validator_signer.as_ref().map(|vs| vs.validator_id());
    ...
    /// 향후 에포크에 대해 진행 중인 모든 상태 동기화를 살펴보고 처리합니다.
    pub fn run_catchup(
        &mut self,
        ...
        file_write("run_catchup():향후 에포크에 대해 진행 중인 모든 상태 동기화를 살펴보고 처리합니다.").unwrap();
    ...
    pub fn process_challenge(&mut self, _challenge: Challenge) -> Result<(), Error> {
        file_write("process_challenge()").unwrap();
        // TODO(2445): Enable challenges when they are working correctly.
    ...
    pub fn run_flat_storage_creation_step(&mut self) -> Result<bool, Error> {
        file_write("run_flat_storage_creation_step():플랫 스토리지 생성 단계 실행").unwrap();
...
impl Client {
    pub fn request_block(&self, hash: CryptoHash, peer_id: PeerId) {
        ...
        match self.chain.block_exists(&hash) {
            ...
            Ok(true) => {
                println!("Hash value:{}",hash.to_string());
