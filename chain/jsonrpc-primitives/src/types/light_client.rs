...
#[derive(Debug, serde::Serialize, serde::Deserialize)]
pub struct RpcLightClientExecutionProofResponse { //거래 결과 증명
    pub outcome_proof: near_primitives::views::ExecutionOutcomeWithIdView, // 실행 결과 증명
    pub outcome_root_proof: near_primitives::merkle::MerklePath,// 샤드 실행 결과 루트 증명
    pub block_header_lite: near_primitives::views::LightClientBlockLiteView, //결과 루트를 포함하는 블록의 light weight 표현
    pub block_proof: near_primitives::merkle::MerklePath, //라이트 클라이언트 헤드까지의 블록으로 구성된 블록 머클 트리에서 블록의 존재를 증명합니다.
}
