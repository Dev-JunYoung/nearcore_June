...
pub trait EpochManagerAdapter: Send + Sync {
    ...
    /// 제안서에서 순서대로 정렬된 에포크 블록 생산자.
    /// 높이가 알려진 경계를 벗어날 경우 EpochError 를 반환합니다.
    ...
    /// 메인 블록의 주어진 높이에 대한 블록 생성자. 알려진 경계를 벗어나면 EpochError 를 반환합니다.
...
impl<T: HasEpochMangerHandle + Send + Sync> EpochManagerAdapter for T {
    ...
    fn get_epoch_id_from_prev_block(
        ...
        log("get_epoch_id_from_prev_block()");
        let epoch_manager = self.read();
    ...
    fn get_epoch_height_from_prev_block(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("get_epoch_height_from_prev_block()");
        let epoch_manager = self.read();
    ...
    fn get_next_epoch_id_from_prev_block(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("get_next_epoch_id_from_prev_block()");
        let epoch_manager = self.read();
    ...
    fn get_epoch_id(&self, block_hash: &CryptoHash) -> Result<EpochId, EpochError> {
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("get_epoch_id():");
        println!("{:?}",epoch_manager.get_epoch_id(block_hash).map_err(EpochError::from));
    ...
    fn get_epoch_start_height(&self, block_hash: &CryptoHash) -> Result<BlockHeight, EpochError> {
        let epoch_manager = self.read();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("get_epoch_start_height():");
        println!("{:?}",epoch_manager.get_epoch_start_height(block_hash).map_err(EpochError::from));
    ...
    fn get_prev_epoch_id_from_prev_block(
        ...
        let epoch_manager = self.read();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("get_prev_epoch_id_from_prev_block():");
        if epoch_manager.is_next_block_epoch_start(prev_block_hash)? {

            println!("{:?}",epoch_manager.get_epoch_id(prev_block_hash).map_err(EpochError::from));
            epoch_manager.get_epoch_id(prev_block_hash).map_err(EpochError::from)
        } else {
            println!("{:?}",epoch_manager.get_prev_epoch_id(prev_block_hash).map_err(EpochError::from));
