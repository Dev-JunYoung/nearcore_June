...
/// 주어진 높이에서 시작하여 주어진 환경에서 `blocks_number` 블록을 생성합니다.
/// 이 연산 후 체인에서 비어있는 첫 번째 높이를 반환합니다.
...
/// 체인에 정체를 유발하는 환경과 트랜잭션 세트를 생성합니다.
...
/// 블록 생성 클라이언트를 실행하고 네트워크 모의가 두 블록을 수신한 후 중지합니다.
#[test]
...
/// 네트워크에서 블록을 수신하고 승인을 받아 네트워크에 헤더를 발표하는 클라이언트를 실행합니다.
/// 승인을 받으려면 블록 생성자 3명이 필요합니다.
#[test]
...
/// 새로 생성된 블록에 다음 블록에 대한 승인을 포함합니다.
#[test]
...
/// 유효하지 않은 블록 한 개와 유효한 블록 한 개를 보내고 클라이언트가 유효한 블록만 발표하는지 확인합니다.
/// 그리고 노드가 유효하지 않은 블록 헤더에 대해 피어를 금지하는지 확인합니다.
