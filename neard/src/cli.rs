...
impl NeardCmd {
    pub(super) fn parse_and_run() -> anyhow::Result<()> {print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("parse_and_run()");

        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        println!("let neard_cmd: Self = clap::Parser::parse();");
        let neard_cmd: Self = clap::Parser::parse();
        // Enable logging of the current thread.
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        println!("let _subscriber_guard = default_subscriber(
        ...
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        println!("let home_dir = neard_cmd.opts.home.clone();");
        ...
        let genesis_validation = if neard_cmd.opts.unsafe_fast_startup {
            print!("line: {},{:?} ",std::line!(), std::thread::current().id());
            println!("GenesisValidationMode::UnsafeFast");
            GenesisValidationMode::UnsafeFast
        } else {
            print!("line: {},{:?} ",std::line!(), std::thread::current().id());
            println!("GenesisValidationMode::Full");
            GenesisValidationMode::Full
        };
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
...
pub(super) enum NeardSubCommand {
    ...
    /// NEAR 노드를 실행합니다
    Run(RunCmd),
    /// Sets up local configuration with all necessary files (validator key, node key, genesis and config)
    /// 필요한 모든 파일(유효성 검사기 키, 노드 키, 제네시스 및 구성)로 로컬 구성을 설정합니다.
    ...
    /// database.
    /// 전체 스토리지를 다시 압축합니다.
    /// 이 작업은 데이터베이스에서 모든 데이터를 읽고 데이터베이스의 새 복사본에 기록하는 느린 작업입니다.
    ///
