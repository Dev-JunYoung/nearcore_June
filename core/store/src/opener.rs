...
impl<'a> StoreOpener<'a> {
    ...
    pub(crate) fn new(
        ...
        file_write("new() impl storeOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("new() impl storeOpener");
        Self {
    ...
    ///
    /// 지정된 [`StoreMigrator`]로 오프너를 구성합니다.
    ///
    /// 마이그레이터가 구성되지 않은 경우, 이전 버전의 데이터베이스를 가지고 있는
    /// 이전 버전의 데이터베이스를 열지 못합니다.  마이그레이터를 구성하면 다음과 같이 합니다.
    /// 마이그레이션을 시도합니다.
    pub fn with_migrator(mut self, migrator: &'a dyn StoreMigrator) -> Self {
        self.migrator = Some(migrator);
        file_write("with_migrator() impl storeOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("with_migrator() impl storeOpener");
        self
    }
    ...
    pub fn open_in_mode(&self, mode: Mode) -> Result<crate::NodeStorage, StoreOpenerError> {
        {
            file_write("open_in_mode").unwrap();
            print!("line: {},{:?} ",std::line!(), std::thread::current().id());
            log("open_in_mode");
    ...
    pub fn create_snapshots(&self, mode: Mode) -> Result<(Snapshot, Snapshot), StoreOpenerError> {
        {
            file_write("create_snapshots() impl StoreOpener").unwrap();
            print!("line: {},{:?} ",std::line!(), std::thread::current().id());
            log("create_snapshots() impl StoreOpener");
    ...
    fn ensure_created(mode: Mode, opener: &DBOpener) -> Result<(), StoreOpenerError> {
        file_write("ensure_created() impl StoreOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("ensure_created() impl StoreOpener");
        let meta = opener.get_metadata()?;
        match meta {
            Some(_) if !mode.must_create() => {
                file_write("info!: db_opener The database exists.").unwrap();
    ...
    fn ensure_kind(
        ...
        file_write("ensure_kind() impl StoreOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("ensure_kind() impl StoreOpener");
    ...
    fn ensure_version(
        ...
        file_write("ensure_version() impl StoreOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("ensure_version() impl StoreOpener");
    ...
    fn open_store(
        ...
        file_write("open_store() impl StoreOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("open_store() impl StoreOpener");
    ...
    fn open_store_unsafe(mode: Mode, opener: &DBOpener) -> Result<Store, StoreOpenerError> {
        file_write("open_store_unsafe() impl StoreOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("open_store_unsafe() impl StoreOpener");
...
impl<'a> DBOpener<'a> {
    ...
    fn new(home_dir: &std::path::Path, config: &'a StoreConfig, temp: Temperature) -> Self {
        file_write("new() impl DBOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("new() impl DBOpener");
    ...
    fn get_metadata(&self) -> std::io::Result<Option<DbMetadata>> {
        file_write("get_metadata() impl DBOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("get_metadata() impl DBOpener");
    ...
    fn open(&self, mode: Mode, want_version: DbVersion) -> std::io::Result<(RocksDB, DbMetadata)> {
        file_write("open() impl DBOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("open() impl DBOpener");
    ...
    fn open_unsafe(&self, mode: Mode) -> std::io::Result<RocksDB> {
        file_write("open_unsafe() impl DBOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("open_unsafe() impl DBOpener");
        let db = RocksDB::open(&self.path, &self.config, mode, self.temp)?;
        Ok(db)
    }
    ...
    fn create(&self) -> std::io::Result<RocksDB> {
        file_write("create() impl DBOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("create() impl DBOpener");
    ...
    fn snapshot(&self) -> Result<Snapshot, SnapshotError> {
        file_write("snapshot() impl DBOpener").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("snapshot() impl DBOpener");
...
pub fn checkpoint_hot_storage_and_cleanup_columns(
    ...
    file_write("checkpoint_hot_storage_and_cleanup_columns()").unwrap();
    print!("line: {},{:?} ",std::line!(), std::thread::current().id());
    log("checkpoint_hot_storage_and_cleanup_columns()");
