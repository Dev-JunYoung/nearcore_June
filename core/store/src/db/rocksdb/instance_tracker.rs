...
impl State {
    ...
    fn try_new_instance(&self, max_open_files: u32) -> Result<(), String> {
        file_write("try_new_instance() impl State").unwrap();
        file_write(":새 RocksDB 인스턴스를 등록하고 주어진 최대 파일 열기 설정 옵션에 대한 제한이 충분한지 확인합니다.").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("try_new_instance() impl State");
        let num_instances = {
        ...
        file_write("info! Opened a new RocksDB instance.").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("info! Opened a new RocksDB instance.");
        info!(target: "db", num_instances, "Opened a new RocksDB instance.");
        Ok(())
    }

    fn close_instance(&self, max_open_files: u32) {
        file_write("close_instance() impl State").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("close_instance() impl State");
        let num_instances = {
            let mut inner = self.inner.lock().unwrap();
        ...
        file_write("info! Closed a new RocksDB instance.").unwrap();
    ...
    fn block_until_all_instances_are_closed(&self) {
        file_write("block_until_all_instances_are_closed() impl State").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("block_until_all_instances_are_closed() impl State");
        let mut inner = self.inner.lock().unwrap();
        while inner.count != 0 {
...
pub(super) fn block_until_all_instances_are_closed() {
    file_write("block_until_all_instances_are_closed()").unwrap();
    print!("line: {},{:?} ",std::line!(), std::thread::current().id());
    log("block_until_all_instances_are_closed() ");
...
impl InstanceTracker {
    ...
    pub(super) fn try_new(max_open_files: u32) -> Result<Self, String> {
        file_write("try_new() impl InstanceTracker").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("try_new() impl InstanceTracker");
