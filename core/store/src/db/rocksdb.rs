...
impl RocksDB {
    ...
    pub fn open(
        ...
        file_write("open() impl RocksDB").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("open() impl RocksDB");
    ...
    fn open_with_columns(
        ...
        file_write("open_with_columns() impl RocksDB").unwrap();
        print!("line: {},{:?} ",std::line!(), std::thread::current().id());
        log("open_with_columns() impl RocksDB");
    ...
    fn open_db(
        ...
        file_write("open_db() impl RocksDB").unwrap();
        file_write(":지정된 열 패밀리가 구성된 데이터베이스를 엽니다.").unwrap();
