...
///TrieKey 에서 값을 얻는 것
/// 루트에 해당하는 RawTrieNodeWithSize의 해시에서 시작합니다.
/// TrieKey의 니블을 사용하여 필요한 노드로 내려갑니다.
/// 기본 RawTrieNode를 추출합니다.
/// 리프 또는 브랜치인 경우 값의 해시를 포함해야 합니다.
/// 해시 및 샤드 ID로 스토리지에서 가치를 얻습니다.
/// Trie는 런타임에서 직접 호출되는 경우가 거의 없으며 TrieUpdate를 사용하여 수정됩니다.


/// Trie node with memory cost of its subtree.
/// 서브트리의 메모리 비용이 있는 트라이 노드입니다.
///
/// memory_usage is serialized, stored and contributes to hash.
/// 메모리 사용량을 직렬화하여 저장하고 해시에 기여합니다.
