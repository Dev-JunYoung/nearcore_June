...
impl<'a> GenesisValidator<'a> {
    pub fn new(
        ...
        log("new() in impl GenesisValidator");
        Self {
            genesis_config,
            total_supply: 0,
    ...
    pub fn validate_processed_records(&mut self) {
        log("validate_processed_records()");
        let validators = self
            .genesis_config
            .validators
            .clone()
