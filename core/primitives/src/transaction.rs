...
pub struct Transaction {
    ...
    /// 논스는 풀에서 트랜잭션의 순서를 결정하는 데 사용됩니다.
    /// signer_id`와 `public_key`의 조합에 따라 증가합니다.
    pub nonce: Nonce,
...
pub enum Action {
    ...
    CreateAccount(CreateAccountAction), // 신규 계정 생성
    /// Sets a Wasm code to a receiver_id
    DeployContract(DeployContractAction), //계약 배포
    FunctionCall(FunctionCallAction), //계약에서 메서드를 호출하는
    Transfer(TransferAction), // 계정 간 토큰 이동을 위한 전송
    Stake(StakeAction), //다음기회에 검증인이 되는 것에 대한 관심을 표명하기 위한.
    AddKey(AddKeyAction), //계정에 키 추가(FullAccess/ FunctionCall Access)
...
#[borsh_init(init)]
pub struct SignedTransaction { //트랜잭션 거래 생성자
