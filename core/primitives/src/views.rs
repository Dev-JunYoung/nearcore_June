...
pub struct BlockHeaderInnerLiteView {/// (+@ 라이트 클라이언트)
    pub height: BlockHeight,
    pub epoch_id: CryptoHash,
    pub next_epoch_id: CryptoHash,
    pub prev_state_root: CryptoHash,
...
impl From<BlockHeader> for BlockHeaderInnerLiteView {
    fn from(header: BlockHeader) -> Self {
        ...
        log("impl From<BlockHeader> for BlockHeaderInnerLiteView");
        match header {
...
pub struct ExecutionOutcomeWithIdView {
    pub proof: MerklePath, // 실행 결과 증명
    pub block_hash: CryptoHash, // 결과 루트를 포함하는 블록의 블록 해시
    pub id: CryptoHash, // 트랜잭션의 ID(트랜잭션 또는 영수증)
    pub outcome: ExecutionOutcomeView, // 실제 결과
    }
...
pub mod validator_stake_view {
    ...
    pub struct ValidatorStakeViewV2 {/// (+@ 라이트 클라이언트)
        pub account_id: AccountId,
        pub public_key: PublicKey,
        #[serde(with = "dec_format")]
        pub stake: Balance,
...
pub struct ValidatorStakeViewV1 {/// (+@ 라이트 클라이언트)
    pub account_id: AccountId,
    pub public_key: PublicKey,
    #[serde(with = "dec_format")]
    pub stake: Balance,
...
////RPC-End point 를 통해
// LightClientBlockView 의 인스턴스를 주기적으로 가져오는 방식으로 작동
pub struct LightClientBlockView { /// (+@ 라이트 클라이언트)
    pub prev_block_hash: CryptoHash,
...
pub struct LightClientBlockLiteView { /// (+@ 라이트 클라이언트)
    pub prev_block_hash: CryptoHash,
    pub inner_rest_hash: CryptoHash,
    pub inner_lite: BlockHeaderInnerLiteView,
}
