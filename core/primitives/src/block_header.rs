...
/// 승인 및 스킵에 대해 다른 블록 승인 부분
#[derive(BorshSerialize, BorshDeserialize, serde::Serialize, Debug, Clone, PartialEq, Eq, Hash)]
pub enum ApprovalInner {/// (+@ 라이트 클라이언트)
    Endorsement(CryptoHash), //보증
    Skip(BlockHeight),
}

/// Block approval by other block producers with a signature
/// 서명을 통한 다른 블록 생성자가 블록을 승인
#[derive(BorshSerialize, BorshDeserialize, serde::Serialize, Debug, Clone, PartialEq, Eq)]
pub struct Approval {
    pub inner: ApprovalInner, //{Endorsement,Skip}
    pub target_height: BlockHeight, // 승인을 사용할 수 있는 특정 블록 높이
    pub signature: Signature, //ED25519(ed25519_dalek::Signature),SECP256K1(Secp256K1Signature),
...
impl ApprovalInner {
    pub fn new(
        parent_hash: &CryptoHash, //s)부모 해쉬
        parent_height: BlockHeight, //s)부모 해쉬
        target_height: BlockHeight, //s)생성할 해쉬 해쉬
    ) -> Self {
...
pub struct BlockHeaderV1 {
    ...
    pub inner_lite: BlockHeaderInnerLite, //sent to 라이트 클라이언트
    pub inner_rest: BlockHeaderInnerRest, //그 외
...
/// 블록헤더 데이터 구조 버전, 버전 간 변경 사항을 문서화.
#[derive(BorshSerialize, BorshDeserialize, serde::Serialize, Debug, Clone, Eq, PartialEq)]
