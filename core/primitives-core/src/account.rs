...
/// Nomicon https://nomicon.io/DataStructures/AccessKey
pub struct AccessKey {
    /// Nonce for this access key, used for tx nonce generation. When access key is created, nonce
    /// is set to `(block_height - 1) * 1e6` to avoid tx hash collision on access key re-creation.
    /// for Tx 논스 생성
    /// Nonce=(block_height-1)*1e6 // 재생성 시 Tx 해시 충돌 방지
    pub nonce: Nonce,

    /// Defines permissions for this access key.
    /// 액세스 키에 대한 권한 정의
    /// enum AccessKeyPermission {FunctionCall(FunctionCallPermission), FullAccess}
...
//https://nomicon.io/DataStructures/AccessKey
pub struct FunctionCallPermission {
    ...
    /// 사용 가능한 잔액 한도(for 액세스 키가 function call gas & 거래수수료 지불)
    #[serde(with = "dec_format")]
